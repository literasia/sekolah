...
@include('admin.cbt.modals._butir-soal')
@include('admin.cbt.modals._preview')
    ...
    $('document').ready(function() {
        ...
        if (`{{ $kelas_id }}` != "" || `{{ $soal_id }}` != "") {
            $('#order-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.cbt.butir-soal') }}",
                    type: "GET",
                    data: {
                        'soal_id' : `{{ $soal_id }}`,
                        'kelas_id' : `{{ $kelas_id }}`,
        ...
        $('#form-butir-soal').on('submit', function (event) {
            ...
            if ($('#action').val() == 'add') {
                url = "{{ route('admin.cbt.butir-soal.store') }}";
                text = "Data sukses ditambahkan";
            }
            if ($('#action').val() == 'edit') {
                url = "{{ route('admin.cbt.butir-soal.update') }}";
        ...
        $('#ok_button').click(function () {
            $.ajax({
                url: '/admin/cbt/butir-soal/hapus/'+ user_id,
                beforeSend: function () {
